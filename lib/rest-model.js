 ...
 * @constructor
 */
function RestResource(pluralModelName, baseUrl, supportPatch) {
  if (!this instanceof RestResource) {
    return new RestResource(pluralModelName, baseUrl, supportPatch);
  }
...
/**
 * Map the patch operation to PATCH /{model}/{id}
 * @param {*} id The id value
 * @param {object} obj The HTTP body
 * @param {function} [cb] The callback function
 */
RestResource.prototype.patch = function (id, obj, cb) {
  request(
    this._req({
      method: 'PATCH',
      uri: this._url + '/' + id,
...
function parseFilters(f) {
  ...
  for (field in f.where) {
    if (typeof f.where[field] === 'object') {
      for (filter in f.where[field]) {
        if (filter === "inq") {
          qs[field + "~in"] = f.where[field][filter].join(",");
        } else {
          qs[field + "~" + filter] = f.where[field][filter];
        }
      }
    } else {
      qs[field + "~eq"] = f.where[field];
    }
  }
