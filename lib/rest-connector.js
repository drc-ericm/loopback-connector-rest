...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  var supportPatch = true;

  if (settings.supportPatch != null) {
    supportPatch = !!settings.supportPatch;
  }

  var connector = new RestConnector(baseURL, supportPatch);
 ...
 * @param {boolean} supportPatch Whether to support PATCH
 * @constructor
 */
function RestConnector(baseURL, supportPatch) {
  this._baseURL = baseURL;
  this._supportPatch = supportPatch;
  this._models = {};
  this._resources = {};
}
...
RestConnector.prototype.define = function defineModel(definition) {
  ...
  this._resources[m] = new RestResource(definition.model.pluralModelName, this._baseURL, this._supportPatch);
};
...
/**
 * Get a model constructor given a model name
 * @param {string} model The model name
 */
RestConnector.prototype.getModel = function getModel(model) {
...
RestConnector.prototype.create = function create(model, data, callback) {
  var idName = this.dataSource.idName(model);
  this.getResource(model).create(data, function (err, body, response) {
    ...
    if (response.statusCode === 200 || response.statusCode === 201) {
      callback && callback(null, body[idName] || body);
    } else {
...
RestConnector.prototype.updateOrCreate = function (model, data, callback) {
  ...
  this.exists(model, idValue, function (err, exists) {
    if (exists) {
...
RestConnector.prototype.save = function save(model, data, callback) {
  ...
  this.getResource(model).update(idValue, data, this.responseHandler(model, callback));
};
...
RestConnector.prototype.updateAttributes = function (model, id, data, callback) {
  ...
  var obj = {};
  obj[idName] = data[idName] = id;

  if (this._supportPatch) {
    this.getResource(model).patch(id, data, this.responseHandler(model, callback));
  } else {
    try {
      var self = this;
      this.getModel(model).findById(id, function (err, m) {
        for (k in m.toObject()) {
          obj[k] = m[k];
        }
        for (k in data) {
        ...
        self.save(model, obj, callback);
      });
    } catch (e) {
      callback("PATCH Unsupported", data);
    }
  }
};